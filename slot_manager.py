...
class SlotManager:
    def __init__(self):
        ...
        for be_id, conf in enumerate(BACKENDS):
            ...
            total_slots += n_slots
        self._all_slots: List[GSlot] = [(be_id, s) for be_id, be in enumerate(self.backends) for s in range(be["n_slots"])]
    ...
    async def acquire_for_request(self, restore_key: Optional[str] = None) -> Tuple[GSlot, asyncio.Lock, Optional[bool]]:
        ...
        if restore_key:
            ...
            log.info("restore_before_chat g=%s key=%s ok=%s", g, (restore_key[:16] if restore_key else None), restored)
